...
def scene_graphs_to_surgery_sg(sgs, entity_of_interest=None, IRRELEVANT_PREDS=None):
    '''
    ...
    for elem in sgs:
        ...
        if entity_of_interest is None and IRRELEVANT_PREDS is None:
            current_sg = {(sub, obj): pred for (sub, pred, obj) in sg if sub != 'none' and obj != 'none'}
        elif entity_of_interest is None:
...
def action_labels_to_surgery_sg(action_labels):
    surgery_sg = []  # Records the timepoints as well as the corresponding change log.
    for elem in action_labels:
        action_label = elem['action_label']
        timepoint = elem['timepoint_idx']
        surgery_sg.append((timepoint, action_label))
    return surgery_sg
...
def surgery_sg_actions_to_memory_str(surgery_sg_actions, current_timepoint, TEMPORAL_STYLE='longshort', DROP_HISTORY=False):
    '''
    Temporal style can be all, long, short, longshort
    '''

    def _get_memory_str(action, timepoint, last_reltimepoint):
        rel_timepoint = current_timepoint - timepoint
        if rel_timepoint == last_reltimepoint:  # add without timepoint
            m_str = f'{action}; '
        else:
    ...
    if TEMPORAL_STYLE == 'short':
        # Only include the most recent 5 changes, formatted as short term memory.
        memory_str += 'Short: '
        for timepoint, action in surgery_sg_actions[-5:]:
            m_str, last_reltimepoint = _get_memory_str(action, timepoint, last_reltimepoint)
            if DROP_HISTORY is not False and random.random() < DROP_HISTORY:
                continue
            memory_str += m_str
    elif TEMPORAL_STYLE == 'long':
        # Only include long term memory, formatted in the long term manner.
        memory_str += 'Long: '
        occurrenced_actions = set()
        for timepoint, action in surgery_sg_actions[:-5]:
            # simplified representation: Only the first occurance of every action is logged. "not" actions are also skipped.
            if action not in occurrenced_actions and not action.startswith('not '):
                occurrenced_actions.add(action)
                m_str, last_reltimepoint = _get_memory_str(action, timepoint, last_reltimepoint)
                if DROP_HISTORY is not False and random.random() < DROP_HISTORY:
                    continue
                memory_str += m_str
