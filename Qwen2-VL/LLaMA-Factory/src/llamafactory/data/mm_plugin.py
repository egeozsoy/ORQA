...
class BasePlugin:
    def __init__(self, image_token: Optional[str], video_token: Optional[str], fix_number_of_image_tokens: Optional[int] = None) -> None:
        self.image_token = image_token
        self.video_token = video_token
        self.fix_number_of_image_tokens = fix_number_of_image_tokens
    ...
    def _preprocess_image(self, image: "ImageObject", **kwargs) -> "ImageObject":
        ...
        """
        highres_needed: bool = kwargs.get("highres_needed", False)  # some images get an exception, mainly if text is needed
        image_resolution: int = kwargs.get("image_resolution") if not highres_needed else 512 * 512
    ...
    def _regularize_images(self, images: Sequence["ImageInput"], **kwargs) -> List["ImageObject"]:
        ...
        for image in images:
            ...
            # highres should be removed again if it was added
            kwargs.pop('highres_needed', None)
    ...
    def _get_mm_inputs(
            ...
            images: Sequence["ImageInput"],
            videos: Sequence["VideoInput"],
            processor: "ProcessorMixin", batch_images_to_idx=None
        ...
        input_dict = {"images": None, 'batch_images_to_idx': batch_images_to_idx}
        if len(images) != 0:
...
class Qwen2vlPlugin(BasePlugin):
    ...
    def process_messages(
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                if self.fix_number_of_image_tokens is not None:
                    # completely different logic will be used to handle the image tokens. we will append a fix number of self.image_tokens, and totally ignore the images. MAke sure all IMAGE_PLACEHOLDER references are removed and replaced with in total fix_number_of_image_tokens
                    image_tokens_str = self.image_token * self.fix_number_of_image_tokens
                    content = content.replace(IMAGE_PLACEHOLDER, image_tokens_str, 1)
                    content = content.replace(IMAGE_PLACEHOLDER, "")
                    num_image_tokens += self.fix_number_of_image_tokens
                    break
        ...
        if len(images) != num_image_tokens and not self.fix_number_of_image_tokens:
            raise ValueError(f"The number of images does not match the number of {IMAGE_PLACEHOLDER} tokens.")
    ...
    def get_mm_inputs(
            ...
            batch_ids: Sequence[List[int]],
            processor: Optional["ProcessorMixin"],
            batch_images_to_idx=None
    ) -> Dict[str, Union[List[int], "torch.Tensor"]]:
        self._validate_input(images, videos)
        return self._get_mm_inputs(images, videos, processor, batch_images_to_idx=batch_images_to_idx)
