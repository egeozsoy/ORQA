...
from llamafactory.model.qwen2_vl.serialization import encode
...
class PointTransformerV3(PointModule):
    def __init__(
        ...
        self.shuffle_orders = shuffle_orders
        self.project_pc_dim = project_pc_dim
        ...
        self.project_pc = nn.Linear(enc_channels[-1], project_pc_dim)
    ...
    def _init_weights(self, dtype, device):
        ...
        :return:
        '''
        if self.project_pc_dim == 1536:
            save_path = Path('~/ORQA/Qwen2-VL/LLaMA-Factory/src/llamafactory/model/qwen2_vl/pointtransformerv3_weights.pth')  # for backwards compatibility
        else:
            save_path = Path(f'~/ORQA/Qwen2-VL/LLaMA-Factory/src/llamafactory/model/qwen2_vl/pointtransformerv3_weights_{self.project_pc_dim}.pth')
        if save_path.exists():
            # load the weights
...
if __name__ == '__main__':
    # custom small version
    # point_transformer = PointTransformerV3(
    #     stride=(2, 2),
    #     enc_depths=(2, 2, 2),
    #     enc_channels=(32, 64, 128),  # normally it would go up up up but we want 32 at the end of the day.
    #     enc_num_head=(2, 4, 8),
    #     enc_patch_size=(1024, 1024, 1024),
    #     cls_mode=True,
    #     project_pc_dim=1024
    # )
    # original, bigger version
    point_transformer = PointTransformerV3(
        cls_mode=True,
        project_pc_dim=1536
    )
    summary(point_transformer, col_names=['num_params', 'trainable'])
    point_transformer._init_weights(dtype=torch.float32, device=torch.device('cuda'))
